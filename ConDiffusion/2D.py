...
def main(cfg):
    ...
    if cfg.mode == "train":
        trainset = load_data("/work/yz886/ANFWI_HNO/data/3D/2.5e6/train2.h5")
        print("Train data min max", trainset['data'].min(), trainset['data'].max(), trainset['data'].shape)

        validset = load_data("/work/yz886/ANFWI_HNO/data/3D/2.5e6/valid2.h5")
        print("Valid data min max", validset['data'].min(), validset['data'].max(), validset['data'].shape)
    if cfg.mode == "eval":
        test_dataset = load_data("/work/yz886/ANFWI_HNO/data/3D/2.5e6/test.h5")
    ...
    if cfg.mode == "train":
        ...
        for i, (loss_train, loss_valid, lr, losses_train, losses_valid, epoch) in enumerate(generator):
            ...
            if (i + 1) % 100 == 0:
                ckpt_filename =  f"ckpt_path_{(i + 1) // 100}_reg.pth"
    ...
    if cfg.mode == "eval":
        ...
        for run_idx in range(1):
            ...
            for i in range(0, num_samples, BATCH_SIZE):
                ...
                x_batch = test_dataset['data'][i:end]  # shape: (B, C, 256)
                x_condition = x_batch[:, :-1, :].to(device)

                conditional_samples = sde.sample_ddim(x_condition, steps=1000)  # shape: (B, 1, 256)
                all_conditional_samples.append(conditional_samples)

            all_conditional_samples = torch.cat(all_conditional_samples, dim=0)  # shape: (500, 1, 256)
