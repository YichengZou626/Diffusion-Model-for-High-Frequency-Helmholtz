...
from neuralop.data.datasets import load_helmholtz
...
pipe = ConfigPipeline(
    [
        YamlConfig(
            "./helmholtz_config.yaml", config_name="default", config_folder="../config"
        ),
...
train_loader, test_loaders, data_processor = load_helmholtz(
...
# Save x, y, and out as .npy files for multiple samples
for sample_idx in range(len(test_samples)):
    ...
    y = data['y'].cpu()
    # Model prediction
    out = model(x.unsqueeze(0).to(device)).cpu().detach().numpy()

    # Save files
    #np.save(os.path.join(results_dir, f"x_sample_{sample_idx + 1}.npy"), x.numpy())
    #np.save(os.path.join(results_dir, f"y_sample_{sample_idx + 1}.npy"), y.numpy())
    np.save(os.path.join(results_dir, f"out_sample_{sample_idx + 1}.npy"), out)

print(f"x, y, and out saved as .npy files in {results_dir}")
...
for sample_idx in range(len(test_samples[:500])):
    data = test_samples[sample_idx]
    data = data_processor.preprocess(data, batched=False)

    x = data['x'].unsqueeze(0).to(device)  # Shape: [1, 2, 64, 64]
    #print(x.shape)
    ...
    # Gradient w.r.t. channel 0 (sound map)
    grad = x.grad.detach().cpu().numpy()  # [2, 64, 64]
    #print(grad.shape)
    grad_channel_0 = np.mean(np.abs(grad), axis=(0, 1)) # shape: [64, 64]

    sensitivity_sum += grad_channel_0
...
plt.colorbar(label='Relative Sensitivity (Sound Map)')
