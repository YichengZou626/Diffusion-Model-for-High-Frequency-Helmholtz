...
class HelmholtzDataset(PTDataset):
    """
    ...
    def __init__(self,
        ...
        available_resolutions = [16, 32, 64, 128, 256]
        ...
        if download:
            ...
            for res in resolutions:
                if f"helmholtz_train_{res}.pt" not in already_downloaded_files or \
                f"helmholtz_test_{res}.pt" not in already_downloaded_files:
                    files_to_download.append(f"helmholtz_{res}.tgz")
            download_from_zenodo_record(record_id=zenodo_record_id,
                                        root=root_dir,
                                        files_to_download=files_to_download)
            
        # once downloaded/if files already exist, init PTDatase
...
def load_helmholtz(n_train,
    ...
    dataset = HelmholtzDataset(root_dir = data_root,
                           n_train=n_train,
                           n_tests=n_tests,
                           batch_size=batch_size,
                           test_batch_sizes=test_batch_sizes,
