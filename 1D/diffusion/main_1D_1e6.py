...
def main(cfg):
    ...
    if cfg.mode == "train":
        trainset = load_data("/work-old/yz886/ANFWI_HNO/data/1D/1e6/train.h5")
        trainset['data'] = trainset['data'][:10000]
        print("Train data min max", trainset['data'].min(), trainset['data'].max(), trainset['data'].shape)

        validset = load_data("/work-old/yz886/ANFWI_HNO/data/1D/1e6/valid.h5")
        print("Valid data min max", validset['data'].min(), validset['data'].max(), validset['data'].shape)
    if cfg.mode == "eval":
        test_dataset = load_data("/work-old/yz886/ANFWI_HNO/data/1D/1e6/perturbation.h5")
