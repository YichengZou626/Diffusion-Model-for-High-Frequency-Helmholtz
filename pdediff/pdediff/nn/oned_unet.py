...
class AttentionBlock(nn.Module):
    def __init__(self, n_channels: int, n_heads: int = 1, d_k: Optional[int] = None, n_groups: int = 1):
        ...
        self.norm = nn.LayerNorm(n_channels)  # More stable than GroupNorm
        self.projection = nn.Linear(n_channels, n_heads * d_k * 3)
        self.output = nn.Linear(n_heads * d_k, n_channels)
        self.scale = max(d_k**-0.5, 1e-3)  # Prevent underflow
        self.n_heads = n_heads
        self.d_k = d_k

    def forward(self, x: torch.Tensor):
        x = torch.nan_to_num(x, nan=0.0)  # Remove NaNs from input
        orig_shape = x.shape
        ...
        q = q / (q.norm(dim=-1, keepdim=True) + 1e-6)  # Normalize Q
        k = k / (k.norm(dim=-1, keepdim=True) + 1e-6)  # Normalize K

        attn = torch.einsum("bihd,bjhd->bijh", q, k) * self.scale
        attn = (attn - attn.max(dim=1, keepdim=True).values).softmax(dim=1)  # Normalize before softmax
...
class Unet(nn.Module):
    ...
    def __init__(
        ...
        activation,
        norm: bool = False,
        ch_mults: Union[Tuple[int, ...], List[int]] = (1, 1, 2, 4),
        is_attn: Union[Tuple[bool, ...], List[bool]] = (False, True, False, True),
        mid_attn: bool = False,
        n_blocks: int = 2,
