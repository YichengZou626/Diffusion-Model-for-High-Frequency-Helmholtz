...
def plot_2d_trajectories(trajectories, idx=0, conditional=False):
    """
    Plot 2D trajectories for a batch of trajectories, showing the first component for each of the 3 time steps.

    Args:
        trajectories (np.ndarray or torch.Tensor): Shape (b, 3, 2, 64, 64)
    """
    # Convert to numpy if the input is a torch Tensor
    if isinstance(trajectories, torch.Tensor):
        trajectories = trajectories.numpy()

    # Squeeze to remove the third dimension and keep only the first component
    b, t, _, h, w = trajectories.shape
    data = trajectories[:, :, 0]  # Select the first component, resulting in shape (b, 3, 64, 64)

    if conditional:
        base_dir = "conditional/" + str(idx) + "/trajectory_outputs"
    else:
        base_dir = "unconditional/" + str(idx) + "/trajectory_outputs"
    ...
    for i in range(b):
        # Create directory for current batch
        batch_dir = os.path.join(base_dir, f"batch_{i + 1}")
        os.makedirs(batch_dir, exist_ok=True)

        for j in range(t):
            # Save the raw numpy array
            np.save(os.path.join(batch_dir, f"timestep_{j + 1}.npy"), data[i, j])
    return
